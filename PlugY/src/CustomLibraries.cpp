...
namespace PlugY {
    ...
    void TCustomDll::saveExtendedSaveFile(Commons::Unit *ptChar, BYTE **data, DWORD *maxSize, DWORD *curSize) {
        if (saveExtendedSaveFileFct)
            saveExtendedSaveFileFct(ptChar, data, maxSize, curSize);
    };

    int TCustomDll::loadExtendedSaveFile(Commons::Unit *ptChar, BYTE *data, DWORD maxSize, DWORD *curSize) {
    ...
    void TCustomDll::saveSharedSaveFile(Commons::Unit *ptChar, BYTE **data, DWORD *maxSize, DWORD *curSize) {
        if (saveSharedSaveFileFct)
            saveSharedSaveFileFct(ptChar, data, maxSize, curSize);
    };

    int TCustomDll::loadSharedSaveFile(Commons::Unit *ptChar, BYTE data[], DWORD maxSize, DWORD *curSize) {
    ...
    void TCustomDll::initialize(DWORD
    offsetDll) {
    offset = offsetDll;
