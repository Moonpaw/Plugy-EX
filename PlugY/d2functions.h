...
F8(STD,  D2Common,10249,10249,10249,10880,11068,10436,11107,10963,23BCC0,	DWORD,		D2InvAddItem, (Inventory* ptInventory, Unit* ptItem, DWORD posX, DWORD posY, DWORD vValueC, DWORD bIsClient, BYTE page))
F8(STD,  D2Common,10250,10250,10250,00000,00000,00000,00000,00000,00000,	DWORD,		D2Common10250, (const char* file, DWORD line, Inventory* ptInventory, DWORD posX, DWORD posY, DWORD vValueC, DWORD bIsClient, BYTE page))
...
F8(STD,  D2Common,10518,10518,10518,10109,10627,10762,10551,10645,2272B0,	void,		D2AddPlayerStat, (Unit* ptChar, DWORD statID, int amount, DWORD index))
F8(STD,  D2Common,10519,10519,10519,11092,10061,10658,10973,10550,225480,	int,		D2GetPlayerStat, (Unit* ptChar, DWORD statID, DWORD index))
F8(STD,  D2Common,10521,10521,10521,10733,10550,10494,10587,10216,2253B0,	int,		D2GetPlayerBaseStat, (Unit* ptChar, DWORD statID, DWORD index))
...
F8(STD,  D2Common,11276,11276,11276,10254,10074,10111,10435,11081,246CA0,	DWORD,		D2GetSkillCost, (Unit* ptChar, int skpoints, DWORD skillID, DWORD curSkillLevel))
...
A8(STD,  D2Common,1F500,1F510,29FA0,71EB0,32AA0,7D2A0,59870,80C40,2552E0,	void,			D2LoadSuperuniques,(DWORD mempool))
...
A8(FAST, D2Client,4BB20,4BB20,521C0,B8CB0,21250,88EA0,54E10,2CE40,E4D80,	DWORD,		D2PrintStat,(Unit* ptItem, Stats* ptStats, DWORD statID, DWORD statIndex, DWORD statValue, LPWSTR lpText))
A8(FAST, D2Client,85A60,84DE0,80430,9EEB0,62070,8B7A0,BF5F0,18820,521C0,	LPWSTR,		D2SetColorPopup, (LPWSTR popupText, DWORD color))
...
A8(FAST, D2Client,00000,00000,00000,73260,5DE40,791A0,143E0,B61F0,78350,	void,		D2SendToServerXX,(DWORD size, BYTE * data))
A8(FAST, D2Client,88940,87CC0,83260,A1F30,65690,8EF00,C2790,1C190,55F20,	void,		D2TogglePage, (DWORD a, DWORD b, DWORD c))
A8(FAST, D2Client,00000,00000,00000,A6520,710C0,A6640,8CD00,90C10,89980,	void,		D2ClickOnStashButton, (DWORD x, DWORD y))
...
A8(STD,  D2Game,00000,00000, 94E0,E0520,E49A0,A6360,2AAE0,BC700,12E860,	Game*,		D2GetGameByClientID, (DWORD clientID))
A8(FAST, D2Game,00000,00000, B0E0,DF250,E36D0,A5080,29820,BB510,12DED0,	void,		D2BroadcastFunction, (Game* ptGame, void* fct, void* param))
A8(FAST, D2Game, C380, C650, C710,41420,A0D50,7D220,8A3E0,DB780,13B280,	DWORD,		D2SendPacket, (void* ptNetClient, LPVOID pData, DWORD size))
A8(FAST, D2Game, D650, D920, DB50,44D00,A3F20,802E0,8D5F0,DD4F0,13C4A0,	void,		D2SetSkillBaseLevelOnClient, (void* ptClient, Unit* ptChar, DWORD skillID, DWORD sLvl, DWORD bRemove))
...
A8(FAST, D2Game,00000,00000,128F0,38D90,43E60,11FF0,D2070,B2F70,15CA40,	DWORD,		D2VerifIfNotCarry1, (Unit* ptItem, ItemsBIN* itemsData, Unit* ptFirstItem))
A8(FAST, D2Game,00000,00000,22070, 1090, 1DF0, 11F0, 1280, 1340,63740,	Room*,		D2TestPositionInRoom, (Room* ptRoom, DWORD x, DWORD y))
A8(FAST, D2Game,00000,00000,3F220,4ABE0,EC7E0,40B90,24950,CDE20,1A09E0,	DWORD,		D2SpawnSuperUnique, (Game* ptGame, Room* ptRoom, DWORD zero1, DWORD x, DWORD y, DWORD minusOne, DWORD superuniqueID, DWORD zero2))
...
A8(FAST, D2Game,4F100,4F500,5B8A0,B9D70,25D50,44950,54810,3A4C0,1335E0,	DWORD,		D2LoadInventory, (Game* ptGame, Unit* pChar, saveBitField* pdata, DWORD p2, DWORD maxSize, DWORD p4, DWORD *ptNbBytesRead))
A8(FAST, D2Game,7BAE0,7BFD0,8BB00,97620,BEF80,93650,E03A0,6DC40,152F60,	Unit*,		D2GameGetObject, (Game* ptGame, DWORD type, DWORD itemNum))
...
F8(STD,  D2Net,	10005,10005,10005,10035,10020,10036,10024,10015,12AE50,	DWORD,		D2SendToServer, (DWORD size, DWORD one, void* data))
...
F8(STD,  Fog,	10023,10023,10023,00000,00000,00000,00000,00000,00000,	void,		D2FogAssertOld, (const char* ptMessage, DWORD eip, DWORD line))
...
F8(FAST, D2Lang,	10003,10003,10003,10002,10004,10010,10011,10011,124E20,	LPWSTR,		D2GetStringFromString, (const char* ptString))
...
F8(STD,  D2Lang,	10010,10010,10010,00000,00000,00000,00000,00000,00000,	DWORD,		D2PrintBigNumber, (LPWSTR ptBuf , DWORD number, DWORD size))
...
F8(FAST, D2Win, 10131,10131,10131,00000,00000,00000,00000,00000,00000,	void,		D2GetPixelRect,(LPWSTR s, DWORD* x, DWORD* y) )
F8(FAST, D2Win, 10132,10132,10132,00000,00000,00000,00000,00000,00000,	DWORD,		D2PrintTextPopup,(LPWSTR s, DWORD x, DWORD y, DWORD uk, DWORD type, DWORD color) )
...
A8(FAST, D2Client,	 D210, D200, D990,00000,00000,00000,00000,00000,00000,	void,		D2SendToServer3, (BYTE type, WORD p))
A8(FAST, D2Game,	7C2C0,7C7B0,8C2E0,00000,00000,00000,00000,00000,1531C0,	NetClient*,	D2GetClient, (Unit* ptUnit, char* lpszErrFile, DWORD ErrLine))
